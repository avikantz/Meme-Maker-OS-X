...
- (void)dragImage:(NSImage *)anImage at:(NSPoint)viewLocation offset:(NSSize)initialOffset event:(NSEvent *)event pasteboard:(NSPasteboard *)pboard source:(id)sourceObj slideBack:(BOOL)slideFlag
{
	...
	NSSize dragSize = NSSizeFromCGSize(CGSizeMake([self bounds].size.width, ([self bounds].size.width)*(dragImage.size.height/dragImage.size.width)));
    [dragImage setSize:dragSize];//change to the size we are displaying
